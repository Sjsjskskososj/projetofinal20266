// POST /api/accounts/:id/withdraw - saca de uma conta do próprio usuário.
import { NextRequest } from "next/server";
import { readDb, writeDb } from "@/lib/storage";
import { withdraw } from "@/lib/bank";
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const tx = withdraw(db, id, Number(body.amount));
    await writeDb(db);

    return jsonCreated({ transaction: tx });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Erro no saque.";
